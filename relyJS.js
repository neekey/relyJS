...
var util = {
    ...
    handleUrl: function( url, localPath ) {
        ...
        url = ( url || '' ).replace( spaceEx, '' ).split( /\s+/ )[ 0 ];
...
var pri = {
    ...
    getAlias: function( url ){
        var spaceEx = /^(\s+)|(\s+)$/g;
        url = ( url || '' ).replace( spaceEx, '' ).split( /\s+/ )[ 1 ];
        return url ? url : '';
    },
    ...
    getRely: function( data ){
        ...
        reqStr = REQ_EX.exec( data ), reqList;
